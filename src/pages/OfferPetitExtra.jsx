...
const getProductIcon = (offer) => {
  ...
  if (title.includes('template') || description.includes('template')) return Layers;
  return Gift;
};
        ...
        if (sessions.length > 0) {
          ...
          if (userSession.offer_generation?.offerChoices?.orderBump1Choices) {
            const choices = userSession.offer_generation.offerChoices.orderBump1Choices.map((choice, idx) => ({
              ...choice, id: choice.id || `bump_${idx}`,
              icon: getProductIcon(choice), badge: choice.productType || choice.badge || 'Bonus',
      ...
      <div className="lg:ml-72 pt-24 lg:pt-12 pb-12">
        <div className="max-w-3xl mx-auto px-4">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-[#1a1a1a] mb-2">Choisis ton Petit Extra</h2>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {offers.map((offer) => (
                <OfferCardNew key={offer.id} offer={offer} icon={offer.icon}
                  isSelected={selectedOffer?.id === offer.id} onSelect={handleSelect} colorScheme="green" />
              ))}
            </div>
          )}
