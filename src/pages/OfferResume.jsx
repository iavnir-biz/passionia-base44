...
import {
  Loader2, Check, Video, FileText, Layers, CheckSquare, Presentation, GraduationCap, Star, Users,
...
const getProductIcon = (offer) => {
  ...
  if (allText.includes('programme') || allText.includes('formation complète') || allText.includes('parcours complet')) return GraduationCap;
  ...
  if (allText.includes('calendrier') || allText.includes('planning')) return Calendar;
  if (allText.includes('idée') || allText.includes('astuce')) return Lightbulb;
  if (allText.includes('challenge') || allText.includes('défi')) return Target;
  return Package;
};
...
export default function OfferResume() {
  ...
  const cardColors = [
    { bg: 'rgba(59,130,246,0.05)', border: 'rgba(59,130,246,0.15)', iconBg: '#3b82f6' },
    { bg: 'rgba(34,197,94,0.05)', border: 'rgba(34,197,94,0.15)', iconBg: '#22c55e' },
    { bg: 'rgba(139,92,246,0.05)', border: 'rgba(139,92,246,0.15)', iconBg: '#8b5cf6' },
    { bg: 'rgba(245,158,11,0.05)', border: 'rgba(245,158,11,0.15)', iconBg: '#f59e0b' }
  ];
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div className="ml-0 lg:ml-72 pt-28 md:pt-24 lg:pt-12 pb-12 relative">
        {/* Neon circles */}
        ...
        <div style={{ position: 'absolute', width: '200px', height: '200px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(236,72,153,0.06) 0%, transparent 70%)', bottom: '15%', left: '5%', filter: 'blur(60px)', pointerEvents: 'none' }} />

        <div className="max-w-3xl mx-auto px-4 relative z-10">
          {/* Title */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
            <h1 className="text-[#1a1a1a] mb-2 text-2xl md:text-3xl font-bold">📋 Résumé de ton offre</h1>
            <p className="text-[#888] text-sm">Voici la gamme complète que tu as construite pour ton activité.</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex justify-center mb-8">
            <GlowButton onClick={handleContinue} size="lg" className="w-full md:w-auto px-6 md:px-12 text-sm md:text-base">
              Découvrir si mon marché est validé <ArrowRight className="w-4 h-4 md:w-5 md:h-5 ml-2" />
            </GlowButton>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
            ...
            style={{ background: 'linear-gradient(135deg, rgba(249,115,22,0.06), rgba(236,72,153,0.04))', border: '1px solid rgba(249,115,22,0.15)' }}
          >
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-6">
            <h2 className="text-lg md:text-xl font-bold text-[#1a1a1a] mb-5 flex items-center gap-2">
              <Check className="w-5 h-5 text-[#22c55e]" /> Tes 4 offres complètes
            </h2>
            ...
            <div className="space-y-3">
              {products.map((product, index) => {
                ...
                return (
                  <div key={product.key} className="rounded-xl p-4 flex items-center gap-3 transition-all hover:shadow-sm"
                       style={{ background: color.bg, border: `1px solid ${color.border}` }}>
                    <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center shadow-sm" style={{ background: color.iconBg }}>
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-[#1a1a1a] mb-0.5 text-sm truncate md:whitespace-normal">{product.label}</h3>
                      <p className="text-xs text-[#888] truncate md:whitespace-normal">{product.data?.title || 'Non défini'}</p>
                    </div>
                    <span className="text-lg md:text-2xl font-bold text-[#1a1a1a] whitespace-nowrap">{product.data?.price || '—'}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="mt-6 relative">
              <div className="blur-sm opacity-40 pointer-events-none bg-[#f8f8f8] border border-[#e5e5e5] rounded-xl p-6">
                <p className="text-[#888] text-sm">Contenu détaillé de chaque offre avec descriptions complètes, livrables, et bénéfices...</p>
              </div>
              <div className="absolute inset-0 flex items-center justify-center">
                <p className="text-[#1a1a1a] font-semibold text-sm px-5 py-2.5 bg-white/95 rounded-xl border border-[#e5e5e5] shadow-sm">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className="mb-8 p-5 md:p-6 rounded-2xl" style={{ background: '#1a1a1a' }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-lg flex-shrink-0 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f97316, #ec4899)' }}>
                <TrendingUp className="w-5 h-5 text-white" />
              </div>
              <div>
                <h2 className="text-white text-base md:text-lg font-bold leading-tight">Ton Potentiel de Revenus Mensuels</h2>
                <p className="text-white/50 text-[10px] md:text-xs">Basé sur une hypothèse d'une vente par jour</p>
              </div>
            </div>

            <div className="text-center py-4 md:py-6">
              <span className="text-4xl md:text-5xl font-bold block" style={{ background: 'linear-gradient(135deg, #f97316, #ec4899)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                {totalMonthly.toLocaleString('fr-FR')} €
              </span>
              <p className="text-white/40 text-sm mt-1">par mois</p>
            </div>

            <button onClick={() => setShowDetail(!showDetail)} className="w-full flex items-center justify-center gap-2 py-3 text-white/60 hover:text-white transition-colors text-sm">
              {showDetail ? <>Cacher le détail <ChevronUp className="w-4 h-4" /></> : <>Afficher le détail <ChevronDown className="w-4 h-4" /></>}
            </button>

            {showDetail && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="border-t border-white/10 pt-4 mt-2 space-y-3">
                {revenues.map((rev) => (
                  <div key={rev.key} className="flex items-center justify-between py-2 px-3 bg-white/5 rounded-lg">
                    <div>
                      ...
                      <p className="text-white/30 text-xs">{rev.conversionLabel}</p>
                    </div>
                    <div className="text-right">
                      <span className="font-bold text-sm" style={{ color: '#f97316' }}>{rev.total.toLocaleString('fr-FR')} €</span>
                      <p className="text-white/30 text-xs">{rev.price} € × {rev.multiplier}</p>
                    </div>
                  </div>
                ))}
                <div className="flex items-center justify-between py-3 px-3 rounded-lg" style={{ background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.2)' }}>
                  <span className="text-white font-bold">Total Mensuel</span>
                  <span className="text-xl font-bold" style={{ color: '#f97316' }}>{totalMonthly.toLocaleString('fr-FR')} €</span>
                </div>
              </motion.div>
            )}
          </motion.div>
